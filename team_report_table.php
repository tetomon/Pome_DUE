...
$sql_report_team = "SELECT cus.* , eng.first_name firstname ,eng.last_name lastname,  report.report_id report_id , report.date_pm date_pm,report.date_sent date_sent , report.date_create date_create , report.comment comment ,report.file_name file_name,report.remark remark from customer_tb cus , engineer_tb eng , report_tb report WHERE cus.customer_id = report.customer_id and eng.eng_id = report.eng_id and eng.team_id = $team_id ORDER BY report.date_pm DESC";
 $query_sql_report_team = mysql_query($sql_report_team);
 $num_report_team = mysql_num_rows($query_sql_report_team);
...
<table class="table table-striped responsive-utilities jambo_table bulk_action" id="datatable">
                    ...
                    <tbody>
                    <? $i=0; 
                      while ($row_report = mysql_fetch_array($query_sql_report_team)) 
                       { $i=$i+1; ?>
